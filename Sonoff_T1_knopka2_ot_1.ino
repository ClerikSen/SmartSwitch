...
String CmndTopic2 = "cmnd/"+nameLs+"/light2";
String StatusTopic_2 = "status/"+nameLs+"/light2";
...
void initConfig(){
  ...
  CmndTopic2 = "cmnd/"+nameLs+"/light2";
  StatusTopic_2 = "status/"+nameLs+"/light2";
  ...
  cmndTopic2 = new char[CmndTopic2.length()+1];
  CmndTopic2.toCharArray(cmndTopic2,CmndTopic2.length()+1);
  statusTopic_2 = new char[StatusTopic_2.length()+1];
  StatusTopic_2.toCharArray(statusTopic_2,StatusTopic_2.length()+1);
}
...
void buttonChangeCallback_2() {
  if (digitalRead(BUTTON_PIN_2) == 0) {
    //Button has been released, trigger one of the two possible options.
    if(millis() - millisSinceChange_2 > 5000){
        longPress_2();
    }else if (millis() - millisSinceChange_2 > 100){
      //Short press
        shortPress_2();
    } 
  }else {
